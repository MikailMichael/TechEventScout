...
function EventCard({ id, title, img, description, date, time, location, link, tags, onFavourite, isFavourited, user }) {
  ...
  return (
    <div className='relative flex flex-col sm:flex-row bg-background-2 rounded-lg max-w-full sm:w-full'>
      {/* Image */}
      {img && (
        <img
          src={img}
          alt={title}
          className="w-full sm:w-70 h-70 object-cover flex-shrink-0 rounded-t-lg sm:rounded-l-lg sm:rounded-t-none"
        />
      )}
